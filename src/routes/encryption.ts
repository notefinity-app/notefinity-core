...
export function publicKeyRoutes(databaseService: DatabaseService, logger: Logger): Router {
  const router = Router();

  // Store user's public key (for optional collaboration features)
  router.post('/store-public-key', async (req: Request, res: Response): Promise<void> => {
    try {
      if (!req.user) {
        res.status(401).json({
          success: false,
      ...
      const existingKeystore = await databaseService.getUserPublicKey(req.user.userId);

      if (existingKeystore) {
        // Update existing key
        const updatedKeystore = await databaseService.updateUserPublicKey(req.user.userId, {
          publicKey,
          keyId,
          algorithm,
        });
  ...
  router.get('/public-key/:userId', async (req: Request, res: Response): Promise<void> => {
    try {
      if (!req.user) {
        res.status(401).json({
          success: false,
  ...
  router.get('/my-public-key', async (req: Request, res: Response): Promise<void> => {
    try {
      if (!req.user) {
        res.status(401).json({
          success: false,
      ...
      const keystore = await databaseService.getUserPublicKey(req.user.userId);
  ...
  router.delete('/my-public-key', async (req: Request, res: Response): Promise<void> => {
    try {
      if (!req.user) {
        res.status(401).json({
          success: false,
      ...
      const deleted = await databaseService.deleteUserPublicKey(req.user.userId);
